...
if (__DEV__) {
  ...
  console.tron = {
    benchmark: Tron.benchmark,
    clear: noop,
    close: noop,
    configure: Tron.configure,
    ...
    log: noop,
    logImportant: noop,
    onCustomCommand: Tron.onCustomCommand,
    overlay: noop,
    reportError: noop,
    send: noop,
    startTimer: Tron.startTimer,
    storybookSwitcher: Tron.storybookSwitcher,
    use: Tron.use,
    useReactNative: Tron.useReactNative,
    warn: noop,
  };
}
    ...
    if (__DEV__) {
      ...
      if (Platform.OS !== 'web') {
        if (this.config.useAsyncStorage) {
          if (Tron.setAsyncStorageHandler) {
      ...
      // clear if we should
      if (this.config.clearOnLoad && Tron.clear) {
        Tron.clear();
      }
    }
  }
